            ...
            impl<R: tauri::Runtime> $crate::Panel<R> for $class_name<R> {
                ...
                fn set_corner_radius(&self, radius: f64) {
                    unsafe {
                        ...
                        // Parche local: sin masksToBounds la subcapa del
                        // webview no se recorta y las esquinas siguen rectas.
                        let _: () = $crate::objc2::msg_send![&*content_layer, setMasksToBounds: true];
                    }
                }
