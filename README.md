...
- 📋 **Pegar texto e imágenes**: `⌘V` pega texto donde esté el cursor, y si el portapapeles trae una imagen (una captura, algo copiado de una web…) se guarda como PNG en el directorio de adjuntos, se inserta como `![imagen](notely://attachments/…)` y la nota salta automáticamente a la vista previa con la imagen a la vista (vuelve a editar con `⌘E`). También se ve en el widget. `⌘C`/`⌘X`/`⌘A` también funcionan en cualquier campo.
